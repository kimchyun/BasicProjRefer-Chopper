...
public enum HomeMenu {
    ...
    QUIT(0, "프로그램을 종료합니다!\n이용해주셔서 감사합니다~\n"),
    MAIN(1, "3.책 정보\t4.책 주문\t5.고객정보\t9.로그아웃\t0.프로그램 종료\n메뉴를 선택하세요: "),
    JOIN(2, "회원가입에 필요한 정보를 입력합니다.\n"),
    BOOK(3, "31.책 목록\t32.책 검색\t33.책 상세\t1.이전메뉴\t9.로그아웃\t0.프로그램 종료\n메뉴를 선택하세요: "),
    ORDER(4, "41.주문하기\t42.주문내역\n"),
    ORDER_BOOK(41, "주문할 책 번호를 입력하세요: "),
    ORDER_LIST(42, "주문내역\n"),
    ...
    BOOK_DETAIL(33, "검색할 책 번호를 입력하세요: "),
    ...
    public static HomeMenu findMenu(int number) {
        Optional<HomeMenu> any = Arrays.stream(values()).filter(menu -> menu.getMenu() == number).findAny();
        if (any.isPresent()) {
            return any.get();
        } else {
